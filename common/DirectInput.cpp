...
{
	char buffer[256];
	sprintf(buffer, "Mouse x: %.2f\nMouse y: %.2f", getCursorPos().x, getCursorPos().y, mouseDX(), mouseDY());
	gGraphics->drawSmallDxFont(buffer, 10, 500); 
}
...
void DirectInput::setCursorPos(Vector pos)
{
	mCursorX = pos.x;
	mCursorY = pos.y;
}
